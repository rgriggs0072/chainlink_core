...
UX Goals
--------
- No giant table by default (audit is optional expander).
- Dropdown-driven edit/add for a single salesperson.
- Bulk upload with preview + apply (upsert only, safe).
- Reassign workflow:
    1) Preview impact (total rows + optional technical breakdown)
    2) Confirm + Apply
    3) Apply reassignment across operational tables ONLY
    4) Deactivate old salesperson contact (history preserved)

Important design rules
----------------------
- Reassignment ≠ rename.
- Old salesperson becomes inactive.
- History preserved.
- New salesperson gets future emails only (must be active).
...
# Reassign preview state
REASSIGN_PREVIEW_STATE_KEY = "sc_reassign_preview_state"
REASSIGN_CONFIRM_KEY = "sc_reassign_confirm"
...
def _render_reassignment(conn, tenant_id: int, contacts_df: pd.DataFrame):
    """
    Reassign salesperson labels across operational tables (OLD -> NEW).

    UX:
    - Old dropdown: active + inactive (you may need to reassign from inactive)
    - New dropdown: active only (can't assign to inactive)
    - Default placeholder ("Select salesperson") prevents accidental selection
    - Preview persists in session_state; Apply only enabled when selection matches preview
    - Preview displays only total record count; table breakdown in an expander

    Safety:
    - Uses BEGIN/COMMIT/ROLLBACK for atomic changes
    - Reassignment ≠ rename; old contact is deactivated; new must already exist/active
    """
    st.subheader("Reassign salesperson (Old → New)")

    old_options = [PLACEHOLDER] + _contact_names(contacts_df, active_only=False)
    new_options = [PLACEHOLDER] + _contact_names(contacts_df, active_only=True)

    if len(old_options) <= 1:
        st.info("Add contacts first.")
        return

    preview_state = st.session_state.get(REASSIGN_PREVIEW_STATE_KEY)

    # -------------------------
    # Selector + Preview
    # -------------------------
    with st.form("sc_reassign_preview_form"):
        ...
        with col2:
            new_name = st.selectbox(
                "New salesperson (active only)",
                options=new_options,
                index=0,
                key="sc_new_name",
            )

        do_preview = st.form_submit_button("Preview impact", type="primary")

    old_norm = old_name.strip().upper() if old_name and old_name != PLACEHOLDER else ""
    new_norm = new_name.strip().upper() if new_name and new_name != PLACEHOLDER else ""

    # Clear preview helper
    c1, c2 = st.columns([1, 3])
    with c1:
        if st.button("Clear preview", key="sc_reassign_clear_preview"):
            st.session_state.pop(REASSIGN_PREVIEW_STATE_KEY, None)
            st.session_state.pop(REASSIGN_CONFIRM_KEY, None)
            st.rerun()
    with c2:
        st.caption("Preview is required before Apply. Preview is saved until cleared or applied.")

    # -------------------------
    # Build preview when requested
    # -------------------------
    if do_preview:
        ...
        st.session_state[REASSIGN_PREVIEW_STATE_KEY] = {
            "old": old_norm,
            "new": new_norm,
            "counts": counts,
            "df": preview_df,
        ...
        st.session_state.pop(REASSIGN_CONFIRM_KEY, None)
        preview_state = st.session_state[REASSIGN_PREVIEW_STATE_KEY]

    # -------------------------
    # Render preview if exists
    # -------------------------
    if not preview_state:
        st.info("Run Preview to see impacted rows.")
        return

    preview_old = preview_state.get("old", "")
    preview_new = preview_state.get("new", "")
    preview_df = preview_state.get("df")

    total = int(preview_df["ROWS_TO_UPDATE"].sum()) if isinstance(preview_df, pd.DataFrame) and not preview_df.empty else 0

    st.info(
        f"This reassignment will update **{total:,} record(s)** from **{preview_old} → {preview_new}**."
    )
    ...
    selection_matches_preview = (old_norm == preview_old) and (new_norm == preview_new)
    if not selection_matches_preview:
        st.warning(
            "Your current selections do not match the saved preview. "
            "Click **Preview impact** again before applying."
        )
    ...
    st.caption(
        "This updates historical records and deactivates the old salesperson. This action cannot be undone."
    )

    confirm = st.text_input("Type REASSIGN to confirm", value="", key=REASSIGN_CONFIRM_KEY)
    apply_disabled = (confirm.strip().upper() != "REASSIGN") or (not selection_matches_preview)
    ...
    if st.button("Apply reassignment", type="primary", disabled=apply_disabled, key="sc_apply_reassign"):
        try:
            ...
            updated = apply_salesperson_reassignment(
                conn,
                tenant_id=tenant_id,
                old_salesperson=preview_old,
                new_salesperson=preview_new,
                update_sales_contacts=False,  # ✅ CRITICAL
            )

            deactivate_contact_by_name(conn, tenant_id=tenant_id, salesperson_name=preview_old)
            ...
            total_updated = int(updated_df["ROWS_UPDATED"].sum()) if not updated_df.empty else 0

            st.success(
                f"✅ Reassignment complete. **{total_updated:,} row(s)** were updated. "
                "Old salesperson is now inactive."
            )
            ...
            st.session_state.pop(REASSIGN_PREVIEW_STATE_KEY, None)
            st.session_state.pop(REASSIGN_CONFIRM_KEY, None)
            st.session_state["sc_old_name"] = PLACEHOLDER
            st.session_state["sc_new_name"] = PLACEHOLDER
            ...
            try:
                ...
                pass
            st.error(f"❌ Apply failed (rolled back): {e}")
            return
