...
def validate_store_numbers_for_chain(
    ...
    """
    Validates that STORE_NUMBER values in an uploaded file match active
    stores for the selected chain in CUSTOMERS. Shared by Distro Grid and
    Reset Schedule.
    ...
    selected in the format dropdown. Reset Schedule has no pivot layout,
    so detection always resolves to "standard" (look for a STORE_NUMBER
    column) for that caller.
...
def format_supplier_by_county(uploaded_file_or_df) -> pd.DataFrame:
    if isinstance(uploaded_file_or_df, pd.DataFrame):
        ...
        df = pd.read_excel(uploaded_file_or_df, engine="openpyxl")
    if df.empty:
        return pd.DataFrame(columns=SUPPLIER_COUNTY_REQUIRED_COLUMNS)
